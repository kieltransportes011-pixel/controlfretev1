...
import { Loader2, Users, Shield, ArrowLeft, Search, Edit2, Ban, Lock, Save, X } from 'lucide-react';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onBack, currentUser }) => {
    ...
    const [stats, setStats] = useState<AdminStats>({ totalUsers: 0, activeProUsers: 0, bannedUsers: 0 });
    ...
    const fetchAdminData = async () => {
        try {
            ...
            alert("Acesso negado ou erro de conexão.");
            onBack();
        } finally {
            setLoading(false);
        }
    ...
    if (loading) {
        return (
            <div className="fixed inset-0 bg-white dark:bg-slate-950 flex flex-col items-center justify-center z-50">
                ...
                <p className="mt-4 text-slate-500 font-medium">Carregando Gestão de Usuários...</p>
            </div>
        );
    }
            ...
            <div className="bg-slate-900 border-b border-slate-800 text-white px-6 py-4 flex justify-between items-center sticky top-0 z-10 shadow-md">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-400">Gestão de Usuários e Planos</p>
                    </div>
                </div>
