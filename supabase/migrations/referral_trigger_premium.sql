-- Function to handle referral rewards (Premium Only)
...
BEGIN
  ...
  IF (OLD.is_premium = false OR OLD.is_premium IS NULL) AND (NEW.is_premium = true) AND (NEW.referred_by IS NOT NULL) THEN
    ...
    SET 
      referral_balance = referral_balance + 20.00, -- Updated reward to R$ 20,00 as it's a premium conversion
...
-- Drop old Trigger (Insert based)
DROP TRIGGER IF EXISTS on_auth_user_created_referral ON public.profiles;
DROP FUNCTION IF EXISTS public.handle_new_referral; -- Clean up old function
